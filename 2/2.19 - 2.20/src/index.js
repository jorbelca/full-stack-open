    ...
    if (window.confirm(`Are you sure to delete the information of ${name}?`)) {
      deletePerson(id)
        ...
        .catch((e) => {
          console.log(e)
          setErrorMessage(
            `Information of ${name} has already benn removed from the server`
          )
        ...
        if (
          ...
          updatePerson(person.id, newPerson)
            ...
            .catch((e) => {
              console.log(e)
              setErrorMessage(
                `Information of ${person.name} has already benn removed from the server`
              )
              setTimeout(() => setErrorMessage(""), 5000)
            })
        }
    ...
    if (persons.filter((person) => person.name === newName).length >= 1) {
      ...
      createPerson(newPerson)
        ...
        .catch((e) => {
          console.log(e)
          setErrorMessage(
            `Information of ${newPerson.name} has already benn removed from the server`
          )
          setTimeout(() => setErrorMessage(""), 5000)
        })
    }
